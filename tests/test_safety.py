...
'''
    ...
    def test_accounts_hardening_is_explicit_and_retains_developer_docker_access(self):
        ...
        credentials = (ROOT / 'lib/accounts.sh').read_text()
        common = (ROOT / 'lib/common.sh').read_text()
        ...
        self.assertIn('--as-user ACCOUNT', bootstrap)
        self.assertIn('Existing account separation retained during delegated root run.', bootstrap)
        self.assertIn('as_workstation_user()', common)
        self.assertIn('sudo usermod -aG docker "$current"', accounts)
        self.assertIn('gpasswd -d "$current" sudo', accounts)
        self.assertNotIn('Type ROOT', credentials)
        self.assertIn('--passwordbox', credentials)
        self.assertIn('sudo chpasswd', credentials)
        self.assertIn('--inputbox', credentials)
    ...
    def test_locks_have_explicit_architecture_and_digest(self):
        ...
        for item in locks.values():
            ...
            if item.get('architecture') == 'all':
                self.assertRegex(item['url'], r'/[a-f0-9]{40}$')
            else:
