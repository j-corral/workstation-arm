...
| Obsidian | [Flathub](https://flathub.org/en/apps/md.obsidian.Obsidian) | Official-team-verified Flatpak is available for aarch64. It provides its own desktop icon and updates; set `OBSIDIAN_DISABLE_GPU=1` in Flatseal if Parallels rendering is unstable. Existing vaults remain untouched. |
...
| Quad9 Secure DoT | [Official service addresses](https://docs.quad9.net/services/), [Quad9 protocol/block/DNSSEC tests](https://docs.quad9.net/FAQs/), [Ubuntu 26.04 resolved.conf](https://manpages.ubuntu.com/manpages/resolute/man5/resolved.conf.5.html) | The previous direct `systemd-resolved` DoT drop-in is migrated into AdGuard's `tls://dns.quad9.net` upstream. The older helper remains only for existing-install compatibility; the network module now uses the local AdGuard chain. Quad9 Secure is threat-blocking, DNSSEC-validating and without ECS. |
| AdGuard Home local | [Official ARM64 release](https://github.com/AdguardTeam/AdGuardHome/releases/tag/v0.107.79), [configuration](https://github.com/AdguardTeam/AdGuardHome/wiki/Configuration), [AdGuard DNS filter](https://github.com/AdguardTeam/AdGuardSDNSFilter) | Pinned official ARM64 tarball and vendor-published SHA-256. AdGuard provides local ad/tracker filtering, user rules and allowlists. `systemd-resolved` retains its stub and forwards ordinary queries to loopback AdGuard; a Docker bridge listener is configured only when Docker is active. Quad9 Secure is the sole normal upstream via DoT, with no ECS/fallback; Quad9 owns DNSSEC validation. Existing AdGuard rules are preserved when secure invariants still hold. Guest runtime validation remains necessary. |
